...
___NOTE: To use TM16xxMatrixGFX, AdafruitGFX needs to be installed. The library checks if AdafruitGFX is present, but depending on your compiler version and settings you may get compilation errors or a warning if AdafruitGFX isn't installed.___
...
- The TM16xx chips offer no support for daisychaining multiple chips, but when separate latch lines are used the data and clock lines can be shared for combined displays. For two wire modules the data line can be shared when using separate clock lines.
...
- The QYF-TM1638 module (TM1638 with common anode display and 4x4 keyboard) is fully supported. Please note that while multiple buttons can be pressed, pressing more than two buttons can give faulty results due to the lack of short-out preventing diodes on the module.
...
- TM1628/TM1668 allow 13 segment mode, TM1623/TM1624 allow 14 segment mode. By combining G1/G2 and or omitting decimal point, these chips can be used to display alphanumeric text on 14-Segment + DP LED displays. For complete usage of all segments of alphanumeric LED displays TM1640 and TM1680 can be used.
